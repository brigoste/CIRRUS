...
std::vector<double> TDMA(const LinearSystem sys)
{
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        ...
        if (itDiag == row.end())
            throw std::runtime_error("TDMA: missing diagonal at row " + std::to_string(i));
        b[i] = itDiag->second;
        ...
        for (auto& kv : row)
        {
            ...
            {
                if (std::abs(kv.second) > 1e-14)
                    throw std::runtime_error(
                        "TDMA: matrix row " + std::to_string(i) +
    ...
    {
        if (std::abs(b[i - 1]) < 1e-20)
            throw std::runtime_error("TDMA: zero pivot at row " + std::to_string(i - 1));
    ...
    if (std::abs(b[n - 1]) < 1e-20)
        throw std::runtime_error("TDMA: zero final pivot at row n-1");
    ...
    for (int i = static_cast<int>(n) - 2; i >= 0; --i)
    {
        ...
        if (std::abs(denom) < 1e-20)
            throw std::runtime_error("TDMA: zero pivot at row " + std::to_string(i));
