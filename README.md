# CIRRUS
##**C**omputational **I**nfrastructure for **R**econfigurable **R**egimes and **U**nified **S**olvers. 

A package working with energy transport in fluid and heat transfer regimes. The main diver for the solver and problem implementation is in c++ with auxiliary scripts for plotting in python.

Solver aims to deal with fluid flows and heat transfer in 1D, 2D, and 3D space using multiple solver types.

Geometry discretization deals only in uniform, cartesian grid layouts. Further updates may seek to expand these capabilites.

Heat transfer implementations will use Dirichlect, Neumann, and Convection (Robin) boundary conditions. 
