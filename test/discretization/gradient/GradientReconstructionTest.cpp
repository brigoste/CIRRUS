...
bool runGradientReconstructionTest()
{
    ...
    for (const std::size_t nx : refinementLevels)
    {
        ...
        const double ggError = computeFaceL2Error( mesh,
                                                   phi,
                                                   ggGradient,
                                                   reconstruction );

        const double lsError = computeFaceL2Error( mesh,
                                                   phi,
                                                   lsGradient,
                                                   reconstruction );
    ...
    {
        const double ggOrder = computeOrder( results[i - 1].ggError, results[i].ggError );

        const double lsOrder = computeOrder( results[i - 1].lsError, results[i].lsError );
