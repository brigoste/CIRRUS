...
{
    if (name == "Linear1D")
        return std::make_unique<OneDLinear>(params);

    if (name == "Quadratic1D")
        return std::make_unique<Quadratic1D>(params);

    if (name == "Sinusoidal2D")
        return std::make_unique<Sinusoidal2D>(params);

    if (name == "AdvectionDiffusion2D")
        return std::make_unique<AdvectionDiffusion2D>(params);
