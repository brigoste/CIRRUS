    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        ...
        for (const auto& [j, value] : row)
        {
            ...
            else
            {
                ...
                {
                    throw std::runtime_error( "TDMA: matrix row "
                                            + std::to_string(i)
        ...
        if (std::abs(b[i]) < 1e-20) { throw std::runtime_error( "TDMA: missing or zero diagonal at row " + std::to_string(i) ); }
    }
